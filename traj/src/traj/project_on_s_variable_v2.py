...
##################### v2_test n-dof : to find common reachable vel #######################
...
print "common_estimated_vel: {}".format( estimated_vel )


fig = plt.figure()   
plt.plot( Frwd_common_vel, 'b-', label='bk_vel')
plt.plot( Frwd_common_vel, 'bo')

plt.plot( Bkwd_common_vel, 'r-', label='fw_vel')
plt.plot( Bkwd_common_vel, 'ro')
...
(trajectory_position_function, trajectory_velocity_function, trajectory_acceleration_function,
 trajectory_jerk_function) = traj.trajectory_for_path_v2(path, estimated_vel, max_positions,  max_velocities, max_accelerations, max_jerks)
