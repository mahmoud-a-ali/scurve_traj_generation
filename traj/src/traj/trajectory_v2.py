...
def trajectory_for_path_v2(path, v_start, v_end, max_velocities, max_accelerations, max_jerks):
...
############################ step 1: find Max Forward velocity ######################   
    ...
    for seg in range(n_segs):
        ...
        tj, ta, tv, s_v_nxt = traj.max_reachable_vel( s1-s0, s_fw_vel[seg], 30.0, j_max, a_max, j_max)
...
############################ step 2: find Max Backward velocity ######################   
    ...
    for seg in range(n_segs):
        ...
        tj, ta, tv, s_v_nxt = traj.max_reachable_vel( s1-s0, s_bk_vel[seg], 30.0, v_max, a_max, j_max)
...
############################ step 3: find final max reachable vel ######################   
    ...
    if s_fw_vel[0] > s_bk_vel[0] or s_bk_vel[-1] > s_fw_vel[-1] :
        raise ValueError("combination of v_start({}) & v_end({}) is not feasible".format(s_fw_vel[0], s_bk_vel[-1]) )                 
...
############################ step 4: use the estimated max reachable velocity  ######################   
    ...
    for segment_i in range(len(path_function.functions)):
        ...
        s_position, s_velocity, s_acceleration, s_jerk = traj.fit_traj_segment(0, s1-s0, s_estimated_vel[segment_i], s_estimated_vel[segment_i+1],  30.0, v_max, a_max, j_max)
